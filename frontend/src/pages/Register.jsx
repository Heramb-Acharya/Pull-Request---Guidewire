...
import { Shield, Phone, KeyRound, ChevronRight, Globe } from 'lucide-react';
...
export default function Register() {
  ...
  const handleSendOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await authApi.sendOtp(phone, name, 'English');
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await authApi.verifyOtp(phone, otp, name, 'English');
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] bg-bg grid-bg flex items-center justify-center px-4 py-12 relative overflow-hidden">
      <div className="orb w-72 h-72 bg-primary/10 top-0 right-0 translate-x-1/2 -translate-y-1/2" />
      <div className="orb w-64 h-64 bg-accent/8 bottom-0 left-0 -translate-x-1/2 translate-y-1/2" />
      ...
      <div className="w-full max-w-md animate-slide-up">
        ...
        <div className="card">
          {step === 'form' ? (
            <form onSubmit={handleSendOtp} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1.5">{t('auth.name_label')}</label>
                <input
                  id="reg-name"
              ...
              <button id="reg-send-otp" type="submit" disabled={loading} className="btn-primary w-full flex items-center justify-center gap-2">
              ...
              <div className="space-y-2">
                <p className="text-xs text-muted text-center mb-3">Or continue as a partner rider</p>
                <button id="login-zepto" type="button" className="btn-secondary w-full flex items-center justify-center gap-2 !text-sm">
                ...
                <button id="login-blinkit" type="button" className="btn-secondary w-full flex items-center justify-center gap-2 !text-sm">
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-4">
              <div className="text-center mb-2">
                ...
                <p className="text-xs text-muted mt-1">Valid for 5 minutes</p>
                {demoOtp && (
              ...
              <button id="reg-verify-otp" type="submit" disabled={loading || otp.length !== 6} className="btn-primary w-full flex items-center justify-center gap-2">
                {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <>
                  {t('auth.btn_verify_register')} <ChevronRight className="w-4 h-4" />
                </>}
              </button>

              <button type="button" onClick={() => { setStep('form'); setOtp(''); setError(''); }} className="text-sm text-muted hover:text-slate-300 w-full text-center">
                ← Change phone number
              </button>
            </form>
          )}
        ...
        <p className="text-center text-sm text-muted mt-6">
          <Link to="/login" className="text-primary hover:text-primary-light font-bold">{t('auth.have_account')}</Link>
        </p>
      </div>
    </div>
  );
